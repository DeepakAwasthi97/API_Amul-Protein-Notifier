...
def load_substore_mapping():
    ...
    for sub in substore_list.substore_info:
        ...
        if isinstance(sub['_id'], list):
            ...
            unique_ids = list(set(id_.strip() for id_ in sub['_id'].split(',') if id_.strip()))
        else:
            unique_ids = [sub['_id']] if sub.get('_id') else []
        
        if len(unique_ids) > 1:
            logger.warning(f"Multiple unique _ids for alias {sub.get('alias')}: {unique_ids}. Using first.")
